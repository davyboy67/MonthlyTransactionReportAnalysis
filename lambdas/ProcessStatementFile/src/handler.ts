...
function extractFileBuffer(event: APIGatewayProxyEvent): Buffer | null {
    ...
    if (contentType.includes('multipart/form-data')) {
        ...
        if (!boundary) {
            console.error('No multipart boundary found in Content-Type:', contentType);
            return null;
        }
        ...
        if (!filePart || !filePart.data) {
            console.error('No "file" part found in multipart body. Parts found:', parts.map(p => p.name));
            return null;
        }
    ...
    try {
        ...
        if (!fileBuffer) {
            return {
                ...
                body: JSON.stringify({ error: "File content is required. Send as multipart/form-data with a 'file' field, or as JSON with a base64-encoded 'fileContent' field." })
            };
        }
