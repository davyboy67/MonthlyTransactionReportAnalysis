...
export class DataAnalysisService implements IDataAnalysisService {
  ...
  constructor(
    transactionInfoHandler: ITransactionInfoHandler,
  ...
  private enhanceTransactionInfo(transactions: ITransaction[]): ITransaction[] {
    transactions.forEach((transaction) => {
      const merchant = this._transactionInfoHandler.resolveMerchant(
      ...
      const category =
        this._transactionInfoHandler.resolveCategory(transaction);
      if (!category) {
        throw new Error(
          `Could not resolve category for transaction: ${transaction.Description}`,
        );
      }
  ...
  async analyseTransactions(
    ...
    if (startDate.getMonth() == 11) {
      ...
      //start of a new month, we actually want the analysis of the month that just ended
    ...
    const enhancedTransactions =
      this.enhanceTransactionInfo(transactionsInRange);
